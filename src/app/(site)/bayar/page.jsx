...
const CARA_BAYAR = [
  'Datangi kasir sambil menyebut nomor meja kamu.',
  'Tunjukkan halaman ini — kasir mencocokkan dengan pesanan yang masuk.',
  'Bayar tunai, QRIS, atau transfer sesuai yang kamu pilih saat memesan.',
 ...
 */
export default async function BayarPage({ searchParams }) {
  const meja = typeof searchParams?.meja === 'string' ? searchParams.meja.trim() : '';
  const backHref = meja ? `/meja?meja=${encodeURIComponent(meja)}` : '/meja';
  ...
  const { data, error } = await supabase.rpc('get_table_bill', { p_table_no: meja });
    ...
    <Shell backHref={backHref}>
      ...
      {!error && orders.length === 0 ? (
        <div className="card p-8 text-center">
          ...
          <Link
            href={`/menu?meja=${encodeURIComponent(meja)}`}
            className="mt-6 inline-flex items-center justify-center rounded-xl bg-brand-600 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-brand-700"
          >
            Mulai pesan
          </Link>
            ...
            <ul className="divide-y divide-slate-100">
              {orders.map((o) => (
                <li key={o.invoice_no} className="p-5">
                  ...
                  <Link
                    href={`/struk/${encodeURIComponent(o.invoice_no)}`}
