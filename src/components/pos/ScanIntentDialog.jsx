...
export default function ScanIntentDialog({ tableNo, billCount = 0, billTotal = 0 }) {
  ...
  const q = `?meja=${encodeURIComponent(tableNo)}`;
  const adaTagihan = billCount > 0;

  const tambah = {
    href: `/menu${q}&mode=tambah`,
    title: 'Tambah Pesanan',
    desc: adaTagihan
      ? `Menempel ke tagihan meja ini — sekarang ${billCount} pesanan, ${rupiah(billTotal)}`
  ...
  const langsung = {
    ...
    desc: `Nomor meja sudah terisi otomatis — Meja ${tableNo}`,
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center p-0 sm:items-center sm:p-4">
      ...
      <div
        ...
        className="relative z-10 max-h-[92vh] w-full max-w-md animate-fade-up overflow-y-auto rounded-t-3xl bg-white p-6 shadow-2xl scroll-slim sm:rounded-3xl"
      >
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm text-slate-500">Mau mulai dari mana?</p>
          </div>
        ...
        {/* Penutup diagram: apa pun cabangnya, bayarnya tetap di akhir. */}
        <p className="mt-5 rounded-xl bg-slate-50 px-4 py-3 text-center text-xs leading-snug text-slate-500">
          Pembayaran tetap diselesaikan <span className="font-semibold text-slate-700">di akhir</span>,
          sekali saja di kasir — termasuk semua tambahan dari meja ini.
        </p>
